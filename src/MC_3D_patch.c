...
// Monte Carlo estimator for the number of 3-dimensional partitions.
//
...
//   * Delta_3 = log p_3(n)-log p_3(n-1) is computed and stored in DELTA3_CSV_FILE;
//   * only final p_3(N) is reported in the terminal console;
...
#define DELTA3_CSV_FILE "MC3D_Delta3.csv"
#define SEED_FILE "MC3D_seed.txt"
...
// -----------------------------------------------------------------------------
// Four-dimensional lattice nodes and open-addressing node sets
// -----------------------------------------------------------------------------

typedef struct {
    uint64_t key;              // 4 packed coordinates, d=0 in low bits.
} Node;
...
static uint64_t g_coord_step[4] = {0, 0, 0, 0};
...
static inline void remove_pending_item(NodeSet *s, int target_idx,
                                       int32_t pending[4], int current_i)
{
    const int last = s->count - 1;
...
static inline void apply_add(ChainState *s, const Node *q, int growth_slot,
                             uint8_t lost_remove_mask, const int32_t lost_remove_idx[4],
                             uint8_t new_growth_mask)
{
    (void)lost_remove_idx;
    ...
    for (int i = 0; i < 4; ++i) {
        if (lost_remove_mask & (uint8_t)(1u << i)) {
            const Node pred = node_minus(q, i);
            if (!nodeset_remove(&s->remove, &pred)) die("Predicted removable node missing");
        }
    }
...
static inline void apply_remove(ChainState *s, const Node *x, int remove_slot,
                                uint8_t lost_growth_mask, const int32_t lost_growth_idx[4],
                                uint8_t new_remove_mask)
{
    (void)lost_growth_idx;
    ...
    for (int i = 0; i < 4; ++i) {
        if (lost_growth_mask & (uint8_t)(1u << i)) {
            const Node succ = node_plus(x, i);
            if (!nodeset_remove(&s->growth, &succ)) die("Predicted growth node missing");
        }
    }
                           ...
                           int lo, int hi)
{
    ...
    if (up) {
        ...
        int32_t lost_remove_idx[4] = {-1, -1, -1};
    ...
    int32_t lost_growth_idx[4] = {-1, -1, -1};
...
int main(void)
{
    ...
    /* Choose the narrowest fixed-width packing once per process.  Four
       coordinates must fit, including the transient growth coordinate N.
       With four coordinates, 16 bits/coordinate is the maximum 64-bit
       representation, making every N <= 65535 safe. */
    {
        unsigned bits = 1;
        while ((UINT64_C(1) << bits) <= (uint64_t)N) ++bits;
        if (bits > 16) die("N is too large for 64-bit 4-coordinate packing");
        g_coord_bits = bits;
    ...
    // Direct independent-run statistics for Delta_3(n) itself.
    // Array index i corresponds to Delta_3(KNOWN_N + 1 + i).
    ...
    if (!run_Delta_mean || !run_Delta_M2 || !run_Delta_count)
        die("Out of memory for Delta_3 error statistics");
...
#pragma omp critical
        ...
        // Direct run-level Delta_3(n) statistics.  This is exactly the local
        // quantity used in the final scientific fit and avoids reconstructing
        // an entire p_3(n) curve just to difference logs afterward.
#if NUM_RUNS > 1
    ...
    if (!(KNOWN_P3 > 0.0L) || !isfinite(KNOWN_P3)) {
        fprintf(stderr, "KNOWN_P3 must be finite and positive\n");
        return EXIT_FAILURE;
    }

    FILE *csv_fp = fopen(DELTA3_CSV_FILE, "w");
    if (!csv_fp) die("Unable to create MC3D_Delta3.csv");
    if (fprintf(csv_fp, "n,Delta_3,stat_err_percentage\n") < 0) {
        fclose(csv_fp);
        die("Unable to write MC3D_Delta3.csv header");
    }

    // Write the direct Delta_3(n) observable and its independent-run statistical
    // error as a percentage.  No Monte Carlo data columns are printed to stdout.
    long double log_p3_reconstructed = logl(KNOWN_P3);
    for (int n = KNOWN_N; n < N; ++n) {
        const int idx_n = n - 1 - first_level;
        const int idx_np1 = n - first_level;
        if (idx_n < 0 || idx_np1 < 0 ||
            ...
            fclose(csv_fp);
            fprintf(stderr, "Missing pooled statistics for Delta_3(%d)\n", n + 1);
            return EXIT_FAILURE;
        }
        ...
        if (!(gp_n > 0.0L) || !(gm_np1 > 0.0L) ||
            !isfinite(gp_n) || !isfinite(gm_np1)) {
            fclose(csv_fp);
            fprintf(stderr, "Invalid pooled broad-histogram averages for Delta_3(%d)\n", n + 1);
            return EXIT_FAILURE;
        }
        ...
        if (fprintf(csv_fp, "%d,%.18Le,%.12Lf\n",
            ...
            fclose(csv_fp);
            die("Unable to write MC3D_Delta3.csv data");
        }

        log_p3_reconstructed += delta3;
    }

    if (fclose(csv_fp) != 0) die("Unable to close MC3D_Delta3.csv");

    // Reconstruct only p_3(N), once, for the console validation/result line.
    printf("Reconstructed p_3(%d) = ", N);
