...
// Monte Carlo estimator for the number of 6-dimensional partitions.
//
//
...
//   * Delta_6 = log p_6(n)-log p_6(n-1) is computed and stored in DELTA6_CSV_FILE;
//   * only final p_6(N) is reported in the terminal console;
...
#define DELTA6_CSV_FILE "MC6D_Delta6.csv"
#define SEED_FILE "MC6D_seed.txt"
...
#ifndef KNOWN_P6
#define KNOWN_P6 14510628853.0L
#endif
...
// -----------------------------------------------------------------------------
// Six-dimensional lattice nodes and open-addressing node sets
// -----------------------------------------------------------------------------

typedef struct {
    __uint128_t key;              // 7 packed coordinates, d=0 in low bits.
} Node;
...
/* Coordinate packing is selected once in main().  For N ~= 15000--20000 this is 15 bits/coordinate, i.e. 105 bits total. */
static unsigned g_coord_bits = 0;
static __uint128_t g_coord_mask = 0;
static __uint128_t g_coord_step[7] = {0, 0, 0, 0, 0, 0, 0};
...
static inline bool nodeset_remove_pending(NodeSet *s, int32_t pending[7], int count)
{
...
static inline void apply_add(ChainState *s, const Node *q, uint64_t qhash, int growth_slot,
                             uint8_t lost_remove_mask, const int32_t lost_remove_idx[7],
                             uint8_t new_growth_mask)
{
...
static inline void apply_remove(ChainState *s, const Node *x, uint64_t xhash, int remove_slot,
                                uint8_t lost_growth_mask, const int32_t lost_growth_idx[7],
                                uint8_t new_remove_mask)
{
                           ...
                           int lo, int hi)
{
    ...
    if (up) {
        ...
        int32_t lost_remove_idx[7] = {-1, -1, -1, -1, -1, -1};
    ...
    int32_t lost_growth_idx[7] = {-1, -1, -1, -1, -1, -1};
...
int main(void)
{
    ...
    /* One-time packing-width selection. Seven coordinates are packed into
       128 bits; for 15000--20000, this uses 15 bits per coordinate. */
    {
        unsigned bits = 1;
        while (((__uint128_t)1 << bits) <= (uint64_t)N) ++bits;
        if (bits > 25) die("N is too large for 128-bit 7-coordinate packing");
        g_coord_bits = bits;
    ...
    // Direct independent-run statistics for Delta_6(n) itself.
    // Array index i corresponds to Delta_6(KNOWN_N + 1 + i).
    ...
    if (!run_Delta_mean || !run_Delta_M2 || !run_Delta_count)
        die("Out of memory for Delta_6 error statistics");
...
#pragma omp critical
        ...
        // Direct run-level Delta_6(n) statistics.  This is exactly the local
        // quantity used in the final scientific fit and avoids reconstructing
        // an entire p_6(n) curve just to difference logs afterward.
#if NUM_RUNS > 1
    ...
    if (!(KNOWN_P6 > 0.0L) || !isfinite(KNOWN_P6)) {
        fprintf(stderr, "KNOWN_P6 must be finite and positive\n");
        return EXIT_FAILURE;
    }

    FILE *csv_fp = fopen(DELTA6_CSV_FILE, "w");
    if (!csv_fp) die("Unable to create MC6D_Delta5.csv");
    if (fprintf(csv_fp, "n,Delta_6,stat_err_percentage\n") < 0) {
        fclose(csv_fp);
        die("Unable to write MC6D_Delta5.csv header");
    }

    // Write the direct Delta_6(n) observable and its independent-run statistical
    // error as a percentage.  No Monte Carlo data columns are printed to stdout.
    long double log_p3_reconstructed = logl(KNOWN_P6);
    for (int n = KNOWN_N; n < N; ++n) {
        const int idx_n = n - 1 - first_level;
        const int idx_np1 = n - first_level;
        if (idx_n < 0 || idx_np1 < 0 ||
            ...
            fclose(csv_fp);
            fprintf(stderr, "Missing pooled statistics for Delta_6(%d)\n", n + 1);
            return EXIT_FAILURE;
        }
        ...
        if (!(gp_n > 0.0L) || !(gm_np1 > 0.0L) ||
            !isfinite(gp_n) || !isfinite(gm_np1)) {
            fclose(csv_fp);
            fprintf(stderr, "Invalid pooled broad-histogram averages for Delta_6(%d)\n", n + 1);
            return EXIT_FAILURE;
        }
        ...
        if (fprintf(csv_fp, "%d,%.18Le,%.12Lf\n",
            ...
            fclose(csv_fp);
            die("Unable to write MC6D_Delta5.csv data");
        }

        log_p3_reconstructed += delta3;
    }

    if (fclose(csv_fp) != 0) die("Unable to close MC6D_Delta5.csv");

    // Reconstruct only p_6(N), once, for the console validation/result line.
    printf("Reconstructed p_6(%d) = ", N);
