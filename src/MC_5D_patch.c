...
// Monte Carlo estimator for the number of 5-dimensional partitions.
//
//
...
//   * Delta_5 = log p_5(n)-log p_5(n-1) is computed and stored in DELTA5_CSV_FILE;
//   * only final p_5(N) is reported in the terminal console;
...
#define DELTA5_CSV_FILE "MC5D_Delta5.csv"
#define SEED_FILE "MC5D_seed.txt"
...
#ifndef KNOWN_P5
#define KNOWN_P5 4043288324470.0L
#endif
...
typedef struct {
    __uint128_t key;              // 6 packed coordinates, d=0 in low bits.
} Node;
...
static __uint128_t g_coord_step[6] = {0, 0, 0, 0, 0, 0};
...
static inline bool nodeset_remove_pending(NodeSet *s, int32_t pending[6], int count)
{
...
static inline void apply_add(ChainState *s, const Node *q, uint64_t qhash, int growth_slot,
                             uint8_t lost_remove_mask, const int32_t lost_remove_idx[6],
                             uint8_t new_growth_mask)
{
...
static inline void apply_remove(ChainState *s, const Node *x, uint64_t xhash, int remove_slot,
                                uint8_t lost_growth_mask, const int32_t lost_growth_idx[6],
                                uint8_t new_remove_mask)
{
                           ...
                           int lo, int hi)
{
    ...
    if (up) {
        ...
        int32_t lost_remove_idx[6] = {-1, -1, -1, -1, -1, -1};
    ...
    int32_t lost_growth_idx[6] = {-1, -1, -1, -1, -1, -1};
...
int main(void)
{
       ...
       128 bits; for 15000--20000, this uses 15 bits per coordinate. */
    {
        ...
        if (bits > 25) die("N is too large for 128-bit 6-coordinate packing");
        g_coord_bits = bits;
    ...
    /*
     * The padded first window is allowed to extend below KNOWN_N, but a
     * lattice level itself can never be negative.  In particular, with
     * KNOWN_N=30 and overlap=32 the raw expression is -3.  Keep the clamp
     * explicit at the point where the window geometry is created so no
     * negative window bound can enter MH or level indexing.
     */
    const int first_level =
        (WL_REQUIRED_LEVEL_SPAN > WL_FIRST_WINDOW_PAD_THRESHOLD)
            ? ((KNOWN_N - 1 - WL_WINDOW_OVERLAP) > 0
                   ? (KNOWN_N - 1 - WL_WINDOW_OVERLAP)
                   : 0)
            : (KNOWN_N - 1);
    if (first_level < 0) die("Internal error: negative first window level");
    ...
    // Direct independent-run statistics for Delta_5(n) itself.
    // Array index i corresponds to Delta_5(KNOWN_N + 1 + i).
    ...
    if (!run_Delta_mean || !run_Delta_M2 || !run_Delta_count)
        die("Out of memory for Delta_5 error statistics");
...
#pragma omp critical
        ...
        // Direct run-level Delta_5(n) statistics.  This is exactly the local
        // quantity used in the final scientific fit and avoids reconstructing
        // an entire p_5(n) curve just to difference logs afterward.
#if NUM_RUNS > 1
    ...
    if (!(KNOWN_P5 > 0.0L) || !isfinite(KNOWN_P5)) {
        fprintf(stderr, "KNOWN_P5 must be finite and positive\n");
        return EXIT_FAILURE;
    }

    FILE *csv_fp = fopen(DELTA5_CSV_FILE, "w");
    if (!csv_fp) die("Unable to create MC5D_Delta5.csv");
    if (fprintf(csv_fp, "n,Delta_5,stat_err_percentage\n") < 0) {
        fclose(csv_fp);
        die("Unable to write MC5D_Delta5.csv header");
    }

    // Write the direct Delta_5(n) observable and its independent-run statistical
    // error as a percentage.  No Monte Carlo data columns are printed to stdout.
    long double log_p3_reconstructed = logl(KNOWN_P5);
    for (int n = KNOWN_N; n < N; ++n) {
        const int idx_n = n - 1 - first_level;
        const int idx_np1 = n - first_level;
        if (idx_n < 0 || idx_np1 < 0 ||
            ...
            fclose(csv_fp);
            fprintf(stderr, "Missing pooled statistics for Delta_5(%d)\n", n + 1);
            return EXIT_FAILURE;
        }
        ...
        if (!(gp_n > 0.0L) || !(gm_np1 > 0.0L) ||
            !isfinite(gp_n) || !isfinite(gm_np1)) {
            fclose(csv_fp);
            fprintf(stderr, "Invalid pooled broad-histogram averages for Delta_5(%d)\n", n + 1);
            return EXIT_FAILURE;
        }
        ...
        if (fprintf(csv_fp, "%d,%.18Le,%.12Lf\n",
            ...
            fclose(csv_fp);
            die("Unable to write MC5D_Delta5.csv data");
        }

        log_p3_reconstructed += delta3;
    }

    if (fclose(csv_fp) != 0) die("Unable to close MC5D_Delta5.csv");

    // Reconstruct only p_5(N), once, for the console validation/result line.
    printf("Reconstructed p_5(%d) = ", N);
