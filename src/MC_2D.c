...
// Monte Carlo estimator for the number of 2-dimensional partitions.
//
...
//   * the final p_2(n) reconstruction is performed only forward from KNOWN_N,
//     entirely in log space.
//
//
//
// Compile with OpenMP, link math library and use optimization flags for fast compiled
// code.
//
//
// -----------------------------------------------------------------------------
...
#define DELTA2_CSV_FILE "MC2D_Delta2.csv"
#define SEED_FILE "MC2D_seed.txt"
...
#define WL_MIN_VISITS_PER_BIN 1000ULL
#endif
#ifndef WL_1T_SCALE
...
#ifndef WL_1T_STEPS
#define WL_1T_STEPS 60000000ULL
#endif
...
#ifndef PROD_BURN_IN
#define PROD_BURN_IN 10000000ULL
#endif
#ifndef PROD_STEPS
#define PROD_STEPS 60000000ULL
#endif
...
// -----------------------------------------------------------------------------
// Three-dimensional lattice nodes and open-addressing node sets
// -----------------------------------------------------------------------------

typedef struct {
    uint64_t key;              // 3 packed coordinates, d=0 in low bits.
} Node;
...
/* Coordinate packing is selected once in main().  For N ~= 15000 this is
   14 bits/coordinate, i.e. 42 bits total. */
static unsigned g_coord_bits = 0;
static uint64_t g_coord_mask = 0;
static uint64_t g_coord_step[3] = {0, 0, 0};
static long double *g_log_inv_count = NULL;
static long double *g_log_half_over_count = NULL;
static size_t g_log_count_cap = 0;

static inline uint64_t coord_mask_for_dim(int d)
{
...
{
    Node n = { (uint64_t)value << (d * g_coord_bits) };
    return n;
}
...
{
    /* The packed key is folded before multiplication so the low table bits
       depend on all three coordinates rather than only x. */
    uint64_t h = a->key ^ (a->key >> 21) ^ (a->key >> 42);
...
static inline void remove_pending_item(NodeSet *s, int target_idx,
                                       int32_t pending[3], int current_i)
{
    const int last = s->count - 1;
    const int pos = s->item_slot[target_idx];
    if (!nodeset_remove_slot(s, pos)) die("Predicted node missing during removal");
    if (target_idx == last) return;
    for (int k = current_i + 1; k < 3; ++k) {
        if (pending[k] == last) {
            pending[k] = target_idx;
            break;
        }
    }
}

static inline void apply_add(ChainState *s, const Node *q, int growth_slot,
                             uint8_t lost_remove_mask, const int32_t lost_remove_idx[3],
                             uint8_t new_growth_mask)
{
    ...
    nodeset_add_absent(&s->remove, q);

    int32_t pending[3] = { lost_remove_idx[0], lost_remove_idx[1], lost_remove_idx[2] };
    for (int i = 0; i < 3; ++i)
        if (lost_remove_mask & (uint8_t)(1u << i))
            remove_pending_item(&s->remove, pending[i], pending, i);

    ++s->level;
    nodeset_add_absent(&s->part_set, q);
...
static inline void apply_remove(ChainState *s, const Node *x, int remove_slot,
                                uint8_t lost_growth_mask, const int32_t lost_growth_idx[3],
                                uint8_t new_remove_mask)
{
    ...
    nodeset_add_absent(&s->growth, x);

    int32_t pending[3] = { lost_growth_idx[0], lost_growth_idx[1], lost_growth_idx[2] };
    for (int i = 0; i < 3; ++i)
        if (lost_growth_mask & (uint8_t)(1u << i))
            remove_pending_item(&s->growth, pending[i], pending, i);

    if (!nodeset_remove(&s->part_set, x)) die("Failed removing partition node");
...
{
    uint8_t rm = 0, gm = 0;
    int32_t ridx[3] = {-1, -1, -1};
    int new_gp, new_gm;
    predict_add(s, q, &new_gp, &new_gm, &rm, ridx, &gm);
    apply_add(s, q, growth_slot, rm, ridx, gm);
}
                           ...
                           int lo, int hi)
{
    ...
    if (up) {
        ...
        int32_t lost_remove_idx[3] = {-1, -1, -1};
    ...
    int32_t lost_growth_idx[3] = {-1, -1, -1};
...
int main(void)
{
    ...
    /* Choose the narrowest fixed-width packing once per process.  N=15000
       uses 14 bits/coordinate (42 total); all partition coordinates are in
       [0, N-1]. */
    {
        unsigned bits = 1;
        while ((UINT64_C(1) << bits) <= (uint64_t)N) ++bits;
        if (bits > 21) die("N is too large for 64-bit 3-coordinate packing");
        g_coord_bits = bits;
        g_coord_mask = (UINT64_C(1) << bits) - 1ULL;
        for (int d = 0; d < 3; ++d)
            g_coord_step[d] = UINT64_C(1) << (d * bits);
    }

    /* Proposal probabilities are always exactly 1/count or 1/(2*count).
       Precompute their logl values once; each MH step then does table loads
       instead of two expensive transcendental calls, with identical arguments
       to the reference implementation. */
    g_log_count_cap = (size_t)(3 * N + 8);
    ...
    const int first_level_raw = (WL_REQUIRED_LEVEL_SPAN > 256)
    ...
    // Direct independent-run statistics for Delta_2(n) itself.
    // Array index i corresponds to Delta_2(KNOWN_N + 1 + i).
    ...
    if (!run_Delta_mean || !run_Delta_M2 || !run_Delta_count)
        die("Out of memory for Delta_2 error statistics");
...
#pragma omp critical
        ...
        // Direct run-level Delta_2(n) statistics.  This is exactly the local
        // quantity used in the final scientific fit and avoids reconstructing
        // an entire p_2(n) curve just to difference logs afterward.
#if NUM_RUNS > 1
    ...
    if (!(KNOWN_P2 > 0.0L) || !isfinite(KNOWN_P2)) {
        fprintf(stderr, "KNOWN_P2 must be finite and positive\n");
        return EXIT_FAILURE;
    }

    FILE *csv_fp = fopen(DELTA2_CSV_FILE, "w");
    if (!csv_fp) die("Unable to create MC2D_Delta2.csv");
    if (fprintf(csv_fp, "n,Delta_2,stat_err_percentage\n") < 0) {
        fclose(csv_fp);
        die("Unable to write MC2D_Delta2.csv header");
    }

    // Write the direct Delta_2(n) observable and its independent-run statistical
    // error as a percentage.  No Monte Carlo data columns are printed to stdout.
    long double log_p2_reconstructed = logl(KNOWN_P2);
    for (int n = KNOWN_N; n < N; ++n) {
        const int idx_n = n - 1 - first_level;
        const int idx_np1 = n - first_level;
        if (idx_n < 0 || idx_np1 < 0 ||
            ...
            fclose(csv_fp);
            fprintf(stderr, "Missing pooled statistics for Delta_2(%d)\n", n + 1);
            return EXIT_FAILURE;
        }
        ...
        if (!(gp_n > 0.0L) || !(gm_np1 > 0.0L) ||
            !isfinite(gp_n) || !isfinite(gm_np1)) {
            fclose(csv_fp);
            fprintf(stderr, "Invalid pooled broad-histogram averages for Delta_2(%d)\n", n + 1);
            return EXIT_FAILURE;
        }

        const long double delta2 = logl(gp_n) - logl(gm_np1);
        ...
        const long double stat_err_pct =
            (delta2 != 0.0L) ? 100.0L * sigma_delta / fabsl(delta2) : 0.0L;

        if (fprintf(csv_fp, "%d,%.18Le,%.12Lf\n",
                    n + 1, delta2, stat_err_pct) < 0) {
            fclose(csv_fp);
            die("Unable to write MC2D_Delta2.csv data");
        }

        log_p2_reconstructed += delta2;
    }

    if (fclose(csv_fp) != 0) die("Unable to close MC2D_Delta2.csv");

    // Reconstruct only p_2(N), once, for the console validation/result line.
    printf("Reconstructed p_2(%d) = ", N);
    if (log_p2_reconstructed <= logl(LDBL_MAX)) {
        printf("%.18Le\n", expl(log_p2_reconstructed));
    } else {
        printf("10^(%.18Lf) [log-space]\n",
               log_p2_reconstructed / logl(10.0L));
    }
